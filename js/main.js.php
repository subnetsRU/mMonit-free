...
(function (window) {
	...
	MONIT.openModal = function (){
	    if (!MONIT.is_null($('modal'))){
		$('modal').removeClass('hidden');
		if (MONIT.is_null($('modalDiv').hasClass('modalShow'))){
    		    $('modalDiv').addClass('modalShow');
		}
	    }else{
		MONIT.console({type: 'error', text: 'openModal: modal div not found'});
	    }
	}

	MONIT.closeModal = function(){
	    if (!MONIT.is_null($('modal'))){
